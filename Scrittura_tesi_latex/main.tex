...
Nella seguente sezione elenco i principali obiettivi di questa tesi e la procedura di lavoro che è stata seguita per raggiungerli.
...
\begin{itemize}
    ...
    \item[-] Selezione di un intervallo temporale tipicamente da 5-10 giorni nel periodo precedente all'evento, per stimare il fondo di rumore strumentale e GCR.

    \item[-] Definizione evento: sul periodo di background si calcola la media $\mu$ e la deviazione standard $\sigma$ per definire una soglia minima di attivazione $\mu + 3\sigma$, metodo Whitman: per ogni canale si sottrae dal flusso la media $\mu$, tutti i punti con flusso inferiore a $\mu + 3\sigma$ si assegna NaN.
