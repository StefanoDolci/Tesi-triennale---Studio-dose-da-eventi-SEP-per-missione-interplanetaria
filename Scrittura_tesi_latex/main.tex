...
\begin{itemize}
    \item[-] Errore sistematico: costante su tutto il canale e proviene dalle calibrazioni dei sensori e si applica al valore di $F_k$ misurato: $\sigma_{sys} = 10-30\%$ tiene conto delle efficienze geometriche e delle cross-calibrazioni tra sensori diversi. 
...
Dopo aver calcolato la fluenza integrata sui dataset dei vari strumenti disponibili per un dato evento, si esegue un fit con i due modelli E-R e la Band function. Il fit viene eseguito in spazio logaritmico per gestire meglio il salto di circa 8 ordini di grandezza; si usa l'algoritmo di Levenberg-Marquardt, per minimizzare il chi-quadro pesato:$$ \chi^2 = \sum_{k=1}^{N} \left(\frac{\log_{10}F_k - \log_{10}\Phi(E_k; \mathbf{p})}{\sigma_{\log,k}}\right)^2 $$
Gli errori in spazio logaritmico sono propagati:
$$\sigma_{\log,k} = \frac{\sigma_{F_k}}{F_k \cdot \ln(10)} $$
Una volta ottenuti i parametri e la matrice di covarianza per propagare le incertezze su uno spettro continuo, dato che ci sono correlazioni tra i parametri, si utilizza un campionamento Monte Carlo parametrico per stimare l'intervallo di confidenza al 68\% (±1$\sigma$) dello spettro. La procedura è la seguente:

\begin{itemize}
    \item[-] Si generano 1000 set di parametri da una distribuzione multivariata normale $$ \mathbf{p}^{(j)} \sim \mathcal{N}(\hat{\mathbf{p}}, C_{\text{stab}}), \qquad j = 1, \ldots, 1000 $$ dove $\hat{\mathbf{p}}$ sono i parametri migliori ottenuti dal fit e $C_{\text{stab}}$ è la matrice di covarianza stabilizzata.
...
\end{itemize}
Si ottengono dunque tre spettri, uno per la mediana $\Phi_{med}$ e due per i limiti di confidenza $[\Phi_{16}, \Phi_{84}] $, si costruisce una tabella con la fluenza $\Phi$ [p·cm$^{-2}$·sr$^{-1}$·MeV$^{-1}$] e l'energia $E$ [MeV]. Questi risultati vengono esportati in SR-NIEL per il calcolo della fluenza residua dietro la schermatura di alluminio.
Infine usando lo spettro differenziale dal miglior fit si calcolano le fluenze integrali omnidirezionali, ovvero la fluenza totale di particelle con energia maggiore di una certa soglia $E_{0}$:$$ J(>E_{0}) = 4\pi \int_{E_{0}}^{\infty} \Phi(E) dE $$ 
