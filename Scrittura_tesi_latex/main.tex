...
\begin{itemize}
    ...
    \item[-] Per ogni set generato di parametri si ricalcola lo spettro su una griglia discreta di punti equispaziati in spazio logaritmico, per standardizzare la procedura per tutti 8 i SEP è stato scelto un range energetico da 0.005 a 1000 MeV: $$ \Phi^{(j)}(E_i) = SpectralModel(E_i; \mathbf{p}^{(j)}) \qquad  i = 1,...,500$$
...
Si ottengono dunque tre spettri, uno per la mediana e due per i limiti di confidenza, questi vengono esportati in SR-NIEL per il calcolo della fluenza residua dietro la schermatura di alluminio, prima 


Per il calcolo della dose, dai parametri ottenuti per ogni fit si genera una tabella 
con la fluenza $\Phi$ [p·cm$^{-2}$·sr$^{-1}$·MeV$^{-1}$] e energia $E$ [MeV], per standardizzare i risultati e poterli confrontare è stato scelto un range 
energetico tra 1-1000 MeV. Come modello della sonda spaziale si considera una schermatura, planare e sferica, di Al per uno spessore equivalente di 10g/cm$^2$, posta davanti ad uno strato di silicio che rappresenta l'elettronica dove verrà poi calcolata la dose. 
